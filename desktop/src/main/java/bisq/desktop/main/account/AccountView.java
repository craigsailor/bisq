...
public class AccountView extends ActivatableView<TabPane, Void> {
  ...
  @FXML
  Tab fiatAccountsTab, altcoinAccountsTab, notificationTab, passwordTab, seedwordsTab, backupTab;
  ...
  public void initialize() {
        ...
        viewPath -> {
          if (viewPath.size() == 3 && viewPath.indexOf(AccountView.class) == 1) {
            if (arbitratorRegistrationTab == null
                && viewPath.get(2).equals(ArbitratorRegistrationView.class))
              navigation.navigateTo(MainView.class, AccountView.class, FiatAccountsView.class);
            else if (mediatorRegistrationTab == null
                && viewPath.get(2).equals(MediatorRegistrationView.class))
              navigation.navigateTo(MainView.class, AccountView.class, FiatAccountsView.class);
            else loadView(viewPath.tip());
          } else {
            resetSelectedTab();
          }
        };
    ...
    keyEventEventHandler =
        event -> {
          if (Utilities.isAltOrCtrlPressed(KeyCode.Y, event) && arbitratorRegistrationTab == null) {
            // TODO: This can be removed once we don't support arbitrators anymore
            arbitratorRegistrationTab =
                new Tab(Res.get("account.tab.arbitratorRegistration").toUpperCase());
            arbitratorRegistrationTab.setClosable(true);
            root.getTabs().add(arbitratorRegistrationTab);

            navigation.navigateTo(
                MainView.class, AccountView.class, ArbitratorRegistrationView.class);
          } else if (Utilities.isAltOrCtrlPressed(KeyCode.R, event)
              && mediatorRegistrationTab == null) {
            // Keypress to open AddMediator
            mediatorRegistrationTab =
                new Tab(Res.get("account.tab.mediatorRegistration").toUpperCase());
            mediatorRegistrationTab.setClosable(true);
            root.getTabs().add(mediatorRegistrationTab);

            navigation.navigateTo(
                MainView.class, AccountView.class, MediatorRegistrationView.class);
          }
        };
  ...
  protected void activate() {
    ...
    if (navigation.getCurrentPath().size() == 2
      ...
      if (arbitratorRegistrationTab != null)
        ...
        navigation.navigateTo(MainView.class, AccountView.class, BackupView.class);
      else navigation.navigateTo(MainView.class, AccountView.class, FiatAccountsView.class);
    }
