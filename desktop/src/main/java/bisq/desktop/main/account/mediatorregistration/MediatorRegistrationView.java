...
import bisq.desktop.main.overlays.windows.UnlockMediatorRegistrationWindow;
...
@FxmlView
public class MediatorRegistrationView extends ActivatableViewAndModel<VBox, MediatorRegistrationViewModel> {
    ...
    private ChangeListener<Mediator> mediatorChangeListener;
    private UnlockMediatorRegistrationWindow unlockMediatorRegistrationWindow;
    ...
    @Inject
    private MediatorRegistrationView(MediatorRegistrationViewModel model, @Named(AppOptionKeys.USE_DEV_PRIVILEGE_KEYS) boolean useDevPrivilegeKeys) {
        super(model);
    ...
    public void initialize() {
        ...
        mediatorChangeListener = (observable, oldValue, mediator) -> updateLanguageList();
    }
    ...
    protected void deactivate() {
        model.myMediatorProperty.removeListener(mediatorChangeListener);
        languagesListView.getItems().removeListener(listChangeListener);
    }

    public void onTabSelection(boolean isSelectedTab) {
        if (isSelectedTab) {
            model.myMediatorProperty.addListener(mediatorChangeListener);
            updateLanguageList();

            if (model.registrationPubKeyAsHex.get() == null && unlockMediatorRegistrationWindow == null) {
                unlockMediatorRegistrationWindow = new UnlockMediatorRegistrationWindow(useDevPrivilegeKeys);
                unlockMediatorRegistrationWindow.onClose(() -> unlockMediatorRegistrationWindow = null)
    ...
    private void buildUI() {
        ...
        addTitledGroupBg(gridPane, gridRow, 4, Res.get("account.tab.mediatorRegistration"));
        TextField pubKeyTextField = addTopLabelTextField(gridPane, gridRow, Res.get("account.mediatorRegistration.pubKey"),
        ...
        final Tuple2<Button, Button> buttonButtonTuple2 = add2ButtonsAfterGroup(gridPane, ++gridRow, Res.get("account.mediatorRegistration.register"), Res.get("account.mediatorRegistration.revoke"));
        ...
        infoLabel.setText(Res.get("account.mediatorRegistration.info.msg"));
    }
    ...
    private void onRemoveLanguage(String locale) {
        ...
        if (languagesListView.getItems().size() == 0) {
            new Popup<>().warning(Res.get("account.mediatorRegistration.warn.min1Language")).show();
    ...
    private void onRevoke() {
        if (model.isBootstrapped()) {
            model.onRevoke(
                    () -> new Popup<>().feedback(Res.get("account.mediatorRegistration.removedSuccess")).show(),
                    (errorMessage) -> new Popup<>().error(Res.get("account.mediatorRegistration.removedFailed",
    ...
    private void onRegister() {
        if (model.isBootstrapped()) {
            model.onRegister(
                    () -> new Popup<>().feedback(Res.get("account.mediatorRegistration.registerSuccess")).show(),
                    (errorMessage) -> new Popup<>().error(Res.get("account.mediatorRegistration.registerFailed",
                            Res.get("shared.errorMessageInline", errorMessage))).show());
        } else {
            new Popup<>().information(Res.get("popup.warning.notFullyConnected")).show();
        }
