...
import bisq.core.disputes.messages.DisputeMessage;
import bisq.core.disputes.messages.DisputeResultMessage;
import bisq.core.disputes.messages.OpenNewDisputeMessage;
import bisq.core.disputes.messages.PeerOpenedDisputeMessage;
import bisq.core.disputes.messages.PeerPublishedDisputePayoutTxMessage;
...
public class DisputeManager implements PersistedDataHost {
    ...
    public void cleanupDisputes() {
        ...
        // If we have duplicate disputes we close the second one (might happen if both traders opened a dispute and mediator
        // was offline, so could not forward msg to other peer, then the mediator might have 4 disputes open for 1 trade)
    ...
    public void sendOpenNewDisputeMessage(Dispute dispute, boolean reOpen, ResultHandler resultHandler, FaultHandler faultHandler) {
        if (!disputes.contains(dispute)) {
            ...
            if (!storedDisputeOptional.isPresent() || reOpen) {
                ...
                NodeAddress peersNodeAddress = dispute.getContract().getMediatorNodeAddress();
    ...
    // mediator sends that to trading peer when he received openDispute request
    private String sendPeerOpenedDisputeMessage(Dispute disputeFromOpener, Contract contractFromOpener, PubKeyRing pubKeyRing) {
        Dispute dispute = new Dispute(
                ...
                disputeFromOpener.getMediatorPubKeyRing(),
    ...
    public DisputeCommunicationMessage sendDisputeDirectMessage(Dispute dispute, String text, ArrayList<Attachment> attachments) {
        ...
        if (isTrader(dispute) ||
                (isMediator(dispute) && !message.isSystemMessage()))
    ...
    // mediator receives that from trader who opens dispute
    ...
    // not dispute requester receives that from mediator
    private void onPeerOpenedDisputeMessage(PeerOpenedDisputeMessage peerOpenedDisputeMessage) {
        ...
        if (!isMediator(dispute)) {
            ...
            errorMessage = "Mediator received peerOpenedDisputeMessage. That must never happen.";
            log.error(errorMessage);
        }
        ...
        if (!messages.isEmpty()) {
            DisputeCommunicationMessage msg = messages.get(0);
            sendAckMessage(msg, dispute.getMediatorPubKeyRing(), errorMessage == null, errorMessage);
        }

        sendAckMessage(peerOpenedDisputeMessage, dispute.getMediatorPubKeyRing(), errorMessage == null, errorMessage);
    }

    // A trader can receive a msg from the mediator or the mediator from a trader. Trader to trader is not allowed.
...
/*
        if (isMediator(disputeResult)) {
            log.error("Mediator received disputeResultMessage. That must never happen.");
            return;
        }
*/
        ...
        try {
            cleanupRetryMap(uid);
            Dispute dispute = disputeOptional.get();
            mediatorPubKeyRing = dispute.getMediatorPubKeyRing();
                ...
                if (payoutTx == null) {
                    if (dispute.getDepositTxSerialized() != null) {
                        ...
                        Transaction signedDisputedPayoutTx = tradeWalletService.traderSignAndFinalizeDisputedPayoutTx(
                                ...
                                disputeResult.getMediatorSignature(),
                                ...
                                disputeResult.getMediatorPubKey()
                        );
            ...
            if (mediatorPubKeyRing != null) {
                ...
                sendAckMessage(disputeCommunicationMessage, mediatorPubKeyRing, success, errorMessage);
            }
        }
    }
    ...
    private boolean isMediator(Dispute dispute) {
        return keyRing.getPubKeyRing().equals(dispute.getMediatorPubKeyRing());
    }

// Not needed since changing from Arbitration to Mediation
/*
    private boolean isMediator(DisputeResult disputeResult) {
        return Arrays.equals(disputeResult.getMediatorPubKey(),
...
*/
    ...
    private Tuple2<NodeAddress, PubKeyRing> getNodeAddressPubKeyRingTuple(Dispute dispute) {
        ...
        if (isTrader(dispute)) {
            receiverPubKeyRing = dispute.getMediatorPubKeyRing();
            peerNodeAddress = dispute.getContract().getMediatorNodeAddress();
        } else if (isMediator(dispute)) {
