...
public class OpenOfferManager
    implements PeerManager.Listener, DecryptedDirectMessageListener, PersistedDataHost {
  ...
  public OpenOfferManager(
    ...
    // In case the app did get killed the shutDown from the modules is not called, so we use a
    // shutdown hook
    Runtime.getRuntime()
        .addShutdownHook(
            new Thread(
  ...
  private void cleanUpAddressEntries() {
    Set<String> openOffersIdSet =
        openOffers.getList().stream().map(OpenOffer::getId).collect(Collectors.toSet());
    btcWalletService
        .getAddressEntriesForOpenOffer()
        .stream()
        .filter(e -> !openOffersIdSet.contains(e.getOfferId()))
        .forEach(
            e -> {
              log.warn(
                  "We found an outdated addressEntry for openOffer {} (openOffers does not contain that "
                      + "offer), offers.size={}",
                  e.getOfferId(),
                  openOffers.size());
  ...
  public void shutDown(@Nullable Runnable completeHandler) {
    ...
    // Normally we use a delay for broadcasting to the peers, but at shut down we want to get it
    // fast out
  ...
  public void removeOpenOffers(List<OpenOffer> openOffers, @Nullable Runnable completeHandler) {
    ...
    openOffersList.forEach(openOffer -> removeOpenOffer(openOffer, () -> {}, errorMessage -> {}));
    if (completeHandler != null)
      UserThread.runAfter(completeHandler::run, size * 200 + 500, TimeUnit.MILLISECONDS);
  }
  ...
  public void onDirectMessage(
      DecryptedMessageWithPubKey decryptedMessageWithPubKey, NodeAddress peerNodeAddress) {
    ...
    if (networkEnvelope instanceof OfferAvailabilityRequest) {
      ...
      if (ackMessage.getSourceType() == AckMessageSourceType.OFFER_MESSAGE) {
        if (ackMessage.isSuccess()) {
          log.info(
              "Received AckMessage for {} with offerId {} and uid {}",
              ackMessage.getSourceMsgClassName(),
              ackMessage.getSourceId(),
              ackMessage.getSourceUid());
        } else {
          log.warn(
              "Received AckMessage with error state for {} with offerId {} and errorMessage={}",
              ackMessage.getSourceMsgClassName(),
              ackMessage.getSourceId(),
  ...
  private void onBootstrapComplete() {
    ...
    // We republish after a bit as it might be that our connected node still has the offer in the
    // data map
    // but other peers have it already removed because of expired TTL.
    // Those other not directly connected peers would not get the broadcast of the new offer, as the
    // first
    // connected peer (seed node) does nto broadcast if it has the data in the map.
    // To update quickly to the whole network we repeat the republishOffers call after a few seconds
    // when we
    // are better connected to the network. There is no guarantee that all peers will receive it but
    // we have
    // also our periodic timer, so after that longer interval the offer should be available to all
    // peers.
    if (retryRepublishOffersTimer == null)
      retryRepublishOffersTimer =
          UserThread.runAfter(
              OpenOfferManager.this::republishOffers, REPUBLISH_AGAIN_AT_STARTUP_DELAY_SEC);
  ...
  public void onAwakeFromStandby() {
    ...
    stopped = false;
    if (!p2PService.getNetworkNode().getAllConnections().isEmpty()) restart();
  }
        ...
        new PlaceOfferProtocol(
            ...
            transaction -> {
              ...
              if (!stopped) {
                ...
                log.debug(
                    "We have stopped already. We ignore that placeOfferProtocol.placeOffer.onResult call.");
              }
            },
  ...
  public void removeOffer(
      Offer offer, ResultHandler resultHandler, ErrorMessageHandler errorMessageHandler) {
    ...
    if (openOfferOptional.isPresent()) {
      ...
      offerBookService.removeOffer(
          offer.getOfferPayload(), () -> offer.setState(Offer.State.REMOVED), null);
    }
  }

  public void activateOpenOffer(
      OpenOffer openOffer, ResultHandler resultHandler, ErrorMessageHandler errorMessageHandler) {
    if (!offersToBeEdited.containsKey(openOffer.getId())) {
      Offer offer = openOffer.getOffer();
      openOffer.setStorage(openOfferTradableListStorage);
      offerBookService.activateOffer(
          offer,
          () -> {
      ...
      errorMessageHandler.handleErrorMessage(
          "You can't activate an offer that is currently edited.");
    }
  }

  public void deactivateOpenOffer(
      OpenOffer openOffer, ResultHandler resultHandler, ErrorMessageHandler errorMessageHandler) {
    Offer offer = openOffer.getOffer();
    openOffer.setStorage(openOfferTradableListStorage);
    offerBookService.deactivateOffer(
  ...
  public void editOpenOfferStart(
      OpenOffer openOffer, ResultHandler resultHandler, ErrorMessageHandler errorMessageHandler) {
    if (offersToBeEdited.containsKey(openOffer.getId())) {
  ...
  public void editOpenOfferPublish(
      Offer editedOffer,
  ...
  public void editOpenOfferCancel(
      OpenOffer openOffer,
      OpenOffer.State originalState,
      ResultHandler resultHandler,
      ErrorMessageHandler errorMessageHandler) {
  ...
  private void handleOfferAvailabilityRequest(OfferAvailabilityRequest request, NodeAddress peer) {
    log.info(
        "Received OfferAvailabilityRequest from {} with offerId {} and uid {}",
        peer,
        request.getOfferId(),
        request.getUid());
    ...
    try {
      ...
      errorMessage =
          "Message validation failed. Error=" + t.toString() + ", Message=" + request.toString();
    ...
    try {
      ...
      if (openOfferOptional.isPresent()) {
        ...
        if (openOffer.getState() == OpenOffer.State.AVAILABLE) {
          Offer offer = openOffer.getOffer();
          if (preferences
              .getIgnoreTradersList()
              .stream()
              .noneMatch(hostName -> hostName.equals(peer.getHostName()))) {
            ...
            if (acceptedArbitrators != null && !acceptedArbitrators.isEmpty()) {
              arbitratorNodeAddress =
                  ArbitratorSelection.getLeastUsedArbitrator(
                          tradeStatisticsManager, arbitratorManager)
                      .getNodeAddress();
              openOffer.setArbitratorNodeAddress(arbitratorNodeAddress);

              // Check also tradePrice to avoid failures after taker fee is paid caused by a too big
              // difference in trade price between the peers. Also here poor connectivity might
              // cause market price API connection losses and therefore an outdated market price.
              try {
              ...
              log.warn(
                  "acceptedArbitrators is null or empty: acceptedArbitrators="
                      + acceptedArbitrators);
      ...
      OfferAvailabilityResponse offerAvailabilityResponse =
          new OfferAvailabilityResponse(request.offerId, availabilityResult, arbitratorNodeAddress);
      log.info(
          "Send {} with offerId {} and uid {} to peer {}",
  ...
  private void sendAckMessage(
      OfferAvailabilityRequest message, NodeAddress sender, boolean result, String errorMessage) {
    ...
    log.info(
        "Send AckMessage for OfferAvailabilityRequest to peer {} with offerId {} and sourceUid {}",
        takersNodeAddress,
        offerId,
        ackMessage.getSourceUid());
    p2PService.sendEncryptedDirectMessage(
        ...
        new SendDirectMessageListener() {
          ...
          public void onArrived() {
            log.info(
                "AckMessage for OfferAvailabilityRequest arrived at takersNodeAddress {}. offerId={}, sourceUid={}",
                takersNodeAddress,
                offerId,
                ackMessage.getSourceUid());
          }
        ...
        UserThread.runAfterRandomDelay(
            () -> {
              if (openOffers.contains(openOffer)) {
                ...
                if (id != null && !openOffer.isDeactivated()) republishOffer(openOffer);
              }
            },
            minDelay,
            maxDelay,
  ...
  private void republishOffer(OpenOffer openOffer) {
    offerBookService.addOffer(
        openOffer.getOffer(),
        () -> {
          if (!stopped) {
            log.debug("Successful added offer to P2P network");
            // Refresh means we send only the dat needed to refresh the TTL (hash, signature and
            // sequence no.)
            if (periodicRefreshOffersTimer == null) startPeriodicRefreshOffersTimer();
          } else {
            log.debug(
                "We have stopped already. We ignore that offerBookService.republishOffers.onSuccess call.");
          }
        },
        errorMessage -> {
          if (!stopped) {
            ...
            retryRepublishOffersTimer =
                UserThread.runAfter(
                    OpenOfferManager.this::republishOffers, RETRY_REPUBLISH_DELAY_SEC);
          } else {
            log.debug(
                "We have stopped already. We ignore that offerBookService.republishOffers.onFault call.");
          }
        });
    openOffer.setStorage(openOfferTradableListStorage);
  }
          ...
          UserThread.runPeriodically(
              () -> {
                if (!stopped) {
                  ...
                  log.debug(
                      "We have stopped already. We ignore that periodicRepublishOffersTimer.run call.");
                }
              },
              REPUBLISH_INTERVAL_MS,
              TimeUnit.MILLISECONDS);
    else log.trace("periodicRepublishOffersTimer already stated");
  }
          ...
          UserThread.runPeriodically(
              () -> {
                if (!stopped) {
                  ...
                  log.debug(
                      "We have stopped already. We ignore that periodicRefreshOffersTimer.run call.");
                }
              },
              REFRESH_INTERVAL_MS,
              TimeUnit.MILLISECONDS);
    else log.trace("periodicRefreshOffersTimer already stated");
  }

  private void refreshOffer(OpenOffer openOffer) {
    offerBookService.refreshTTL(
        openOffer.getOffer().getOfferPayload(),
